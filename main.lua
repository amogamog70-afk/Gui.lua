-- ╔══════════════════════════════════════════════════════════╗-- ║         METEOR UI LIBRARY — FULL ARCHITECTURE v2.0       ║-- ║   Button · Toggle · Slider · Dropdown · TextBox          ║-- ║   ColorPicker · Keybind · Label · Separator              ║-- ╚══════════════════════════════════════════════════════════╝

local TweenService     = game("TweenService")local UserInputService = game("UserInputService")local Players          = game("Players")local RunService       = game("RunService")local LocalPlayer      = Players.LocalPlayer

-- ── ТЕМА ────────────────────────────────────────────────────local Theme = {-- фоныMainBg      = Color3.fromRGB(10, 10, 14),ElementBg   = Color3.fromRGB(17, 17, 23),ElementHov  = Color3.fromRGB(24, 24, 33),PopupBg     = Color3.fromRGB(13, 13, 18),-- акцентAccent      = Color3.fromRGB(218, 43, 172),AccentDim   = Color3.fromRGB(140, 28, 110),AccentHov   = Color3.fromRGB(240, 60, 195),-- текстTextPri     = Color3.fromRGB(255, 255, 255),TextSec     = Color3.fromRGB(120, 120, 132),TextDis     = Color3.fromRGB(60,  60,  72),-- UIBorder      = Color3.fromRGB(28, 28, 40),TrackBg     = Color3.fromRGB(38, 38, 52),ToggleOff   = Color3.fromRGB(42, 42, 56),}

-- ── УТИЛИТЫ ─────────────────────────────────────────────────local function Tween(obj, props, t, style, dir)TweenService(obj,TweenInfo.new(t or 0.14, style or Enum.EasingStyle.Quad, dir or Enum.EasingDirection.Out),props)()end

local function Corner(parent, radius)local c = Instance.new("UICorner")c.CornerRadius = UDim.new(0, radius or 4)c.Parent = parentreturn cend

local function Stroke(parent, color, thick)local s = Instance.new("UIStroke")s.Color         = color or Theme.Borders.Thickness     = thick or 1s.ApplyStrokeMode = Enum.ApplyStrokeMode.Borders.Parent        = parentreturn send

local function Label(parent, text, size, color, font, xa, ya)local l = Instance.new("TextLabel")l.BackgroundTransparency = 1l.Text          = text or ""l.TextSize      = size or 11l.TextColor3    = color or Theme.TextPril.Font          = font  or Enum.Font.Gothaml.TextXAlignment = xa   or Enum.TextXAlignment.Leftl.TextYAlignment = ya   or Enum.TextYAlignment.Centerl.Parent        = parentreturn lend

-- hover-эффект на кнопкообразных фреймахlocal function HoverFX(btn, bgNorm, bgHov)btn.MouseEnter(function() Tween(btn, {BackgroundColor3 = bgHov  or Theme.ElementHov}) end)btn.MouseLeave(function() Tween(btn, {BackgroundColor3 = bgNorm or Theme.ElementBg }) end)end

-- ── ПОПАПЫ ──────────────────────────────────────────────────local ScreenGui = Instance.new("ScreenGui")ScreenGui.Name           = "MeteorLib"ScreenGui.ResetOnSpawn   = falseScreenGui.DisplayOrder   = 999ScreenGui.ZIndexBehavior = Enum.ZIndexBehavior.SiblingScreenGui.Parent         = LocalPlayer("PlayerGui")

local PopupLayer = Instance.new("Frame")   -- прозрачный слой поверх всегоPopupLayer.Size                 = UDim2.new(1,0,1,0)PopupLayer.BackgroundTransparency = 1PopupLayer.ZIndex               = 50PopupLayer.Parent               = ScreenGui

local function CloseAllPopups()for _, v in ipairs(PopupLayer()) do v() endend

-- закрыть попап по клику вне егоPopupLayer.InputBegan(function(i)if i.UserInputType == Enum.UserInputType.MouseButton1 thenCloseAllPopups()endend)

-- ── TOP BAR ─────────────────────────────────────────────────--   Прижат к самому верху (Y = 0), высота 34 pxlocal BAR_W  = 530local BAR_H  = 34local PAGE_H = 170

local TopBar = Instance.new("Frame")TopBar.Name            = "TopBar"TopBar.Size            = UDim2.new(0, BAR_W, 0, BAR_H)TopBar.Position        = UDim2.new(0.5, -BAR_W/2, 0, 0)   -- ← верхний край экранаTopBar.BackgroundColor3 = Theme.MainBgTopBar.BorderSizePixel = 0TopBar.ZIndex          = 10TopBar.Parent          = ScreenGuiCorner(TopBar, 0)   -- без скруглений сверху (прижат к краю)Stroke(TopBar, Theme.Border, 1)

-- нижняя розовая линияlocal BarLine = Instance.new("Frame")BarLine.Size            = UDim2.new(1, 0, 0, 1)BarLine.Position        = UDim2.new(0, 0, 1, -1)BarLine.BackgroundColor3 = Theme.AccentBarLine.BorderSizePixel = 0BarLine.ZIndex          = 11BarLine.Parent          = TopBar

-- логотип/иконка слеваlocal LogoLabel = Instance.new("TextLabel")LogoLabel.Size            = UDim2.new(0, 70, 1, 0)LogoLabel.Position        = UDim2.new(0, 8, 0, 0)LogoLabel.BackgroundTransparency = 1LogoLabel.Text            = "✦ meteor"LogoLabel.TextColor3      = Theme.AccentLogoLabel.Font            = Enum.Font.GothamBoldLogoLabel.TextSize        = 12LogoLabel.TextXAlignment  = Enum.TextXAlignment.LeftLogoLabel.ZIndex          = 11LogoLabel.Parent          = TopBar

-- контейнер вкладок (после логотипа)local TabContainer = Instance.new("Frame")TabContainer.Size               = UDim2.new(0, 310, 1, -2)TabContainer.Position           = UDim2.new(0, 84, 0, 0)TabContainer.BackgroundTransparency = 1TabContainer.ZIndex             = 11TabContainer.Parent             = TopBar

local TabLayout = Instance.new("UIListLayout")TabLayout.FillDirection  = Enum.FillDirection.HorizontalTabLayout.SortOrder      = Enum.SortOrder.LayoutOrderTabLayout.Padding        = UDim.new(0, 2)TabLayout.VerticalAlignment = Enum.VerticalAlignment.CenterTabLayout.Parent         = TabContainer

-- строка поиска справаlocal SearchBox = Instance.new("Frame")SearchBox.Size            = UDim2.new(0, 120, 0, 20)SearchBox.Position        = UDim2.new(1, -128, 0.5, -10)SearchBox.BackgroundColor3 = Theme.ElementBgSearchBox.BorderSizePixel = 0SearchBox.ZIndex          = 11SearchBox.Parent          = TopBarCorner(SearchBox, 4)Stroke(SearchBox, Theme.Border, 1)

local SearchIcon = Instance.new("ImageLabel")SearchIcon.Size            = UDim2.new(0, 12, 0, 12)SearchIcon.Position        = UDim2.new(0, 5, 0.5, -6)SearchIcon.BackgroundTransparency = 1SearchIcon.Image           = "rbxassetid://11868577178784 3"   -- твоя лупаSearchIcon.ImageColor3     = Theme.TextSecSearchIcon.ZIndex          = 12SearchIcon.Parent          = SearchBox

local SearchInput = Instance.new("TextBox")SearchInput.Size            = UDim2.new(1, -22, 1, 0)SearchInput.Position        = UDim2.new(0, 20, 0, 0)SearchInput.BackgroundTransparency = 1SearchInput.Font            = Enum.Font.GothamSearchInput.Text            = ""SearchInput.PlaceholderText = "Search.."SearchInput.PlaceholderColor3 = Theme.TextSecSearchInput.TextColor3      = Theme.TextPriSearchInput.TextSize        = 10SearchInput.TextXAlignment  = Enum.TextXAlignment.LeftSearchInput.ZIndex          = 12SearchInput.ClearTextOnFocus = falseSearchInput.Parent          = SearchBox

-- ── PAGE CONTAINER (сразу под баром, Y = 34) ────────────────local PageContainer = Instance.new("Frame")PageContainer.Name            = "Pages"PageContainer.Size            = UDim2.new(0, BAR_W, 0, PAGE_H)PageContainer.Position        = UDim2.new(0.5, -BAR_W/2, 0, BAR_H)PageContainer.BackgroundColor3 = Theme.MainBgPageContainer.BorderSizePixel = 0PageContainer.ClipsDescendants = truePageContainer.ZIndex          = 9PageContainer.Parent          = ScreenGuiCorner(PageContainer, 0)Stroke(PageContainer, Theme.Border, 1)

-- ── DRAG (перетаскивание за TopBar) ─────────────────────────dolocal dragging, dragStart, startPosTopBar.InputBegan(function(i)if i.UserInputType == Enum.UserInputType.MouseButton1 thendragging  = truedragStart = i.PositionstartPos  = TopBar.Positionendend)UserInputService.InputChanged(function(i)if dragging and i.UserInputType == Enum.UserInputType.MouseMovement thenlocal delta = i.Position - dragStartlocal newPos = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X,startPos.Y.Scale, startPos.Y.Offset + delta.Y)TopBar.Position        = newPosPageContainer.Position = UDim2.new(newPos.X.Scale, newPos.X.Offset,newPos.Y.Scale, newPos.Y.Offset + BAR_H)PopupLayer()endend)UserInputService.InputEnded(function(i)if i.UserInputType == Enum.UserInputType.MouseButton1 then dragging = false endend)end

-- ════════════════════════════════════════════════════════════-- LIBRARY CORE-- ════════════════════════════════════════════════════════════local Library     = {}local Pages       = {}local AllElements = {}   -- для поискаlocal ActivePage  = nillocal FirstActivate = nil

-- скрыть/показать по RightShiftlocal guiVisible = trueUserInputService.InputBegan(function(i, gp)if gp then return endif i.KeyCode == Enum.KeyCode.RightShift thenguiVisible = not guiVisibleTopBar.Visible         = guiVisiblePageContainer.Visible  = guiVisibleif not guiVisible then CloseAllPopups() endendend)

-- ── ПОИСК ───────────────────────────────────────────────────SearchInput("Text")(function()local q = SearchInput.Text()CloseAllPopups()for _, e in ipairs(AllElements) doif q == "" thene.Frame.Visible = trueelseif string.find(e.Name, q, 1, true) thene.Frame.Visible = truee.Activate()elsee.Frame.Visible = falseendendend)

-- ════════════════════════════════════════════════════════════-- CreateTab-- ════════════════════════════════════════════════════════════function Library(name, icon)
...
-- ── INIT ─────────────────────────────────────────────────────task.spawn(function()repeat task.wait() until FirstActivateFirstActivate()end)

return Library

--[[════════════════════════════════════════════════ПРИМЕР ИСПОЛЬЗОВАНИЯ:════════════════════════════════════════════════

local Lib = loadstring(game("..."))()

local Combat = Lib("Combat", "⚔")Combat("Kill Aura", function() print("on") end)Combat("Auto Block", false, function(v) print(v) end)Combat("Reach", 3, 20, 6, 0.5, function(v) print(v) end)Combat("Target", {"Nearest","Random","Highest HP"}, function(v) print(v) end)Combat("Weapons", {"Sword","Bow","Axe"}, function(t) print(t) end)Combat("Toggle Aura", Enum.KeyCode.V, function() print("toggled") end)

local Visual = Lib("Visual", "👁")Visual("ESP")Visual("Player ESP", true, function(v) end)Visual("ESP Color", Color3.fromRGB(218,43,172), function(c) end)Visual("Watermark", "Enter text..", function(t) end)

Lib("Meteor Loaded", "Enjoy your session!", 4)
